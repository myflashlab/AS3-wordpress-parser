...
// ----------------------------------------------------------------------------------------------------------------------- funcs
		...
		private function onScrollMove(e:ScrollEvent):void
		{
			_scroller._mask.smoothing = true;
			checkForDataRequest();
		}
		
		private function onScrollDone(e:ScrollEvent):void
		{
		...
		public function update($posts:Array):void
		{
			...
			for (var i:int = 0; i < $posts.length; i++) 
			{
				...
				if (!foundDuplicated && $posts[i].type == "post")
				{
					...
					//excerpt.addEventListener(AppEvent.REQUEST_POST, onShowContent);
